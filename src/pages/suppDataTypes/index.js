...
class SuppDataTypes extends Component {
  ...
  componentWillMount() {
    ...
    this.props.fetchType(this.brandId)
  }
  ...
  handleCancel(event) {
    ...
    this.setState({isLoading: true, changeError: false, renderChangeError: false, error: false, isEditing: null, typeValues: []})
    this.props.fetchType(this.brandId)
  }
    ...
    if(this.state.typeValues.length > 0 || this.state.deleteValues.length > 0 ) {
      if(this.state.validateTypes.length <= 0) {
        ...
        _.map(this.state.deleteValues, type => {
          this.props.deleteType(this.brandId, type)
        })
        this.props.createType(this.state.typeValues)
        event.target.value === 'next' ? this.props.history.push(`/suppDataPrice/${this.brandId}`) : this.setState({changeError: false, renderChangeError: false, isEditing: null})
        this.state.progressBar++
      }
    } else {
      if(this.state.validateTypes.length > 0) {
  ...
  handleCheckbox(event, { name }) {
    if(this.state[name] === name) {
      ...
      this.setState({
        ...
        typeValues: [...this.state.typeValues, {brand: this.brandId, product: name}],
